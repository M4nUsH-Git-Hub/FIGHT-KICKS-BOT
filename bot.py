...
async def fetch_sneaker_by_sku(sku: str) -> dict | None:
    """Cerca un prodotto su SneakerAPI tramite SKU. Restituisce i dati o None."""
    import aiohttp
    api_key = os.environ.get("RAPIDAPI_KEY")
    if not api_key:
        return None

    url = "https://sneakerapi-realtime-api.p.rapidapi.com/stockx/search"
    headers = {
        "x-rapidapi-host": "sneakerapi-realtime-api.p.rapidapi.com",
...
async def wtb(interaction: discord.Interaction, sku: str, taglia: str, canale: discord.TextChannel):
    ...
    name = product.get("name") or product.get("title") or "Prodotto sconosciuto"
    image_url = product.get("image") or product.get("thumbnail") or product.get("imageUrl") or ""
    stockx_url = product.get("url") or product.get("link") or f"https://stockx.com/search?s={sku}"
